...
All prices and thresholds are reconciled against the providers' public pages as of April 15, 2026. If you're reading this later, double-check the numbers. The mechanics will still hold.
...
DeepSeek is interesting right now precisely as a counterexample. For `deepseek-v4-flash` as of mid-April 2026, a cache hit costs `$0.0035/MTok`, a cache miss `$0.16/MTok`, output `$0.32/MTok`. That's not the usual 10× discount — closer to 45×.
...
| gpt-5.4-mini | $0.70/M | $0.07/M | $4.20/M | cheap workhorse |
| gpt-5.4-nano | $0.18/M | $0.018/M | $1.10/M | high-volume simple tasks |
...
| DeepSeek V4 Flash | $0.16/M | $0.0035/M | $0.32/M | disk cache, automatic |
| DeepSeek V4 Pro | $0.48/M | $0.0084/M | $0.96/M | disk cache, automatic |
...
### Does the cache affect answer quality?

In a normal model — no. The cache reuses intermediate computations for already-read prefix tokens; decoding runs fresh. With the same prompt, the answer should be distributed the same as without caching, modulo the usual sampling randomness.

If "quality changed after caching," it's almost always the prompt that changed, not the cache. For example, RAG was moved earlier, a dynamic block was removed, tool order was changed, or traffic started going through a different backend.

### Can I just put everything stable into the system prompt?

Sometimes yes, more often no.

The system prompt is convenient for durable instructions. But cramming 200k tokens of RAG into it makes the system huge, hard to manage, and difficult to version. On Anthropic with explicit breakpoints you can cache individual content blocks. On Gemini explicit there's a dedicated cache object. On OpenAI it's often easier to keep the shared corpus at the start of `messages` before the user-specific tail.

The rule isn't "everything into system" — it's "cacheable content should precede dynamic content and have stable serialization."

### What if every user is unique?

Some part of the prefix is still shared: base system, safety/policy, tools, output schema, product documentation, common examples. That part can live in cache, while user-specific context goes below. You won't get 95% hit rate this way, but a steady 30–60% is realistic. On expensive input that's already meaningful money.

### Is it worth caching short prompts?

Often you can't — you bump into the minimum threshold. Even when you can, the savings rarely justify the observability overhead.

A sub-1k-token prompt with a long answer — caching isn't the main lever. Look at output length, model selection, batching, structured output, reasoning settings.

### Why do cached tokens still count toward rate limits?

For most providers the cache reduces compute/cost/latency, but the request doesn't become free from a quota standpoint. Rate-limit policies are checked separately. A high-load system can't be designed on the assumption that cached tokens don't pressure limits unless the provider gives an explicit guarantee.

### What about RAG?

If retrieved chunks differ on every request, that's dynamic content. Don't expect high cache hits on RAG.

If you have a large shared corpus (documentation, a policy manual, a schema catalog), it makes sense to lift it into a stable cacheable block. The retrieved subset or user-specific quotes go below.

The most common mistake here is `[system] → [user question] → [RAG]`. For prefix caching that's a bad layout: the user question breaks the prefix before RAG, and the big RAG never gets reused. Better: `[system] → [stable corpus/RAG anchor] → [user question + retrieved delta]`, when product logic allows.

### Should I use semantic cache instead of prompt cache?

That's a different task.

Prompt cache reuses computations for the same prefix. Semantic cache tries to reuse an answer (or part of it) for a request that's similar in meaning. The first barely affects correctness — the model still sees the same prompt. The second can be wrong on meaning.

Semantic cache is useful for FAQ, search, and extraction with repeating intents. It is not a replacement for prompt caching in agent loops, tool definitions, or long dialogues.

### How do I know optimization isn't needed?

Do the math.

If a cache miss on a late step costs $0.002, traffic is modest, TTFT is fine, and dynamic tool selection visibly helps quality — don't add architectural complexity.

Cache discipline matters when the miss is expensive: lots of input, many steps, an expensive model, high RPS, or a tight TTFT. In other situations it's just decent hygiene, no heroics needed.

---

## Wrap-up

Prompt caching doesn't speed up bad architecture. It makes good architecture cheaper and faster.

When the start of the request is stable, routing is predictable, the TTL is matched to traffic, and tools don't drift step to step — the cache becomes an almost-free win across three axes at once: bill lower, TTFT lower, throughput higher.

When the start of the request lives its own life, prompt caching stays a pretty line in the docs. The invoice doesn't show it.

The most useful question before the next "let's switch to a cheaper model":

```text
What's our effective cost at the real hit rate?
```

And the one nobody likes more:

```text
What happens to that hit rate after migration?
```

If your logs don't answer both — migration turns into an experiment paid out of your pocket. The experiment might go well; I wouldn't count on it.

If you want a tool that runs this audit for you, the skill I built for exactly this is here: **[cache-cop](https://github.com/izum286/cache-cop)** — Claude Code skill for diagnosing why LLM cache reuse fails: prefix stability, provider telemetry, agent tool stability, routing, Bedrock/OpenRouter quirks. Clone, fork, contribute, use in prod - MIT License. Go wild.
...
- OpenAI: [tool_search guide](https://developers.openai.com/api/docs/guides/tools-tool-search)
...
- Anthropic: [Tool search and defer loading](https://platform.claude.com/docs/en/agents-and-tools/tool-use/tool-search-tool)
...
- arXiv: [Don't Break the Cache](https://arxiv.org/abs/2601.06007)
- Claude Code: [How Claude Code uses prompt caching](https://code.claude.com/docs/en/prompt-caching)
- LMCache: [Context Engineering & Reuse Pattern Under the Hood of Claude Code](https://blog.lmcache.ai/en/2025/12/23/context-engineering-reuse-pattern-under-the-hood-of-claude-code/)
- [**cache-cop**](https://github.com/izum286/cache-cop) — the skill mentioned above.
